...
//        product.getParts().addAll(assparts);
            ...
            if(product.getId()!=0) {
                ...
                if(product.getInv()- product2.getInv()>0) {
                    for (Part p : product2.getParts()) {
                        ...
                        if(p.getInv() < p.getMinInv()) {//invWarning = true;
                            bindingResult.rejectValue("inv", "invWarning",  "Fail: The part " + p.getName() + " is low on stock");
                            return "productForm";
                        }
                        partService1.save(p);
                    }
            ...
            repo.save(product);
            //if (invWarning) return  "invWarning";
