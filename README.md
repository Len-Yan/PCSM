

## Project Access on AWS: base-app-env-1.eba-iqr5up2f.us-east-1.elasticbeanstalk.com

Test data and test login included during first start up, only for demo and testing purpose.

The main Project was build without AI in 2023

This is a simplified version of PC-shop management system, frontend is not included. <br>

The Web-service is coded based on Java Spring Framework with built-in H2 Sql database. <br>

The service is host on AWS Beanstalk with public access. <br>

The project is mainly focused on Backend practice, using minimum front end elements. <br>
